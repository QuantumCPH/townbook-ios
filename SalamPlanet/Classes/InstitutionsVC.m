...
@synthesize delegate;
-(id)init{
    self = [super initWithNibName:@"InstitutionsVC" bundle:nil];
    if (self) {
...
-(void)dolocalizationText{
    self.lblPageTitle.text=NSLocalizedString(@"INSTITUTIONS", nil);
}
...
- (void)getShopsList
{
    ...
    [[WebManager sharedInstance] getShopsList:^(NSArray *resultArray) {
        ...
        NSPredicate * customPredicate = [NSPredicate predicateWithFormat:@"self.categoryName = %@",@"Institution"];
        ...
        if (mainArray.count == 0)
            ShowMessage(kAppName, NSLocalizedString(@"No institutions for this Town",nil));
        [self.tableView reloadData];
    ...
    if(isBM)
    {
        [[WebManager sharedInstance] markEntity:shop.entityId isShop:YES
            ...
            if (response!= nil)
            {
                ...
                ShowMessage(kAppName, NSLocalizedString(@"This institution added to your favourities", nil));
            }
        } failure:^(NSString *errorString) {
            [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
            ShowMessage(kAppName,errorString);
