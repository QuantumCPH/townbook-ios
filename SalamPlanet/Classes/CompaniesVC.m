...
@implementation CompaniesVC
@synthesize delegate;
-(id)init{
    self = [super initWithNibName:@"CompaniesVC" bundle:nil];
    if (self) {
...
- (void)getShopsList
{
    ...
    [[WebManager sharedInstance] getShopsList:^(NSArray *resultArray) {
        ...
        NSPredicate * customPredicate = [NSPredicate predicateWithFormat:@"self.categoryName = %@",@"Company"];
    ...
    if(isBM)
    {
        [[WebManager sharedInstance] markEntity:shop.entityId isShop:YES
            ...
            if (response!= nil)
            {
                ...
                ShowMessage(kAppName, NSLocalizedString(@"This company added to your favourities", nil));
            }
        } failure:^(NSString *errorString) {
            [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
            ShowMessage(kAppName,errorString);
    ...
    else
    {
        [[WebManager sharedInstance] markEntity:shop.entityId isShop:YES
            ...
            if (response!= nil)
            {
                ...
                [[DataManager sharedInstance] saveContext];
                ShowMessage(kAppName, NSLocalizedString(@"This company removed from your favourities", nil));
            }
        } failure:^(NSString *errorString) {
            [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
            ShowMessage(kAppName,errorString);
