...
- (void)addParallaxWithImage:(UIImage *)image andHeight:(CGFloat)height andShadow:(BOOL)shadow {
    ...
    {
        APParallaxView *parallaxView = [[APParallaxView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, height) andShadow:shadow];//width*2 removed by Saad
        ...
        [self addSubview:parallaxView.button];
...
- (void)addParallaxWithoutButtonWithImage:(UIImage *)image andHeight:(CGFloat)height andShadow:(BOOL)shadow {
    if(self.parallaxView) {
        if(self.parallaxView.currentSubView) {
            [self.parallaxView.currentSubView removeFromSuperview];
        }
...
- (id)initWithFrame:(CGRect)frame andShadow:(BOOL)shadow {
    if(self = [super initWithFrame:frame]) {
        ...
        [self setBackgroundColor:[UIColor whiteColor]];//[UIColor colorWithRed:127.0/255.0 green:127.0/255.0 blue:127.0/255.0 alpha:1.0]];
        ...
        if (shadow) {
            ...
            [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[shadowView(0.0)]|" options:NSLayoutFormatAlignAllBottom metrics:nil views:@{@"shadowView" : self.shadowView}]];
        ...
        _button=[[UIButton alloc]initWithFrame:CGRectMake(0, -40, 320, 40)];
        
        [_button setTitle:NSLocalizedString(@"Add photo", nil) forState:UIControlStateNormal];
        [_button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_button setTitle:NSLocalizedString(@"Add photo", nil) forState:UIControlStateHighlighted];
        [_button setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
        [_button setTitle:NSLocalizedString(@"Add photo", nil) forState:UIControlStateSelected];
        ...
        [_button addTarget:self action:@selector(addPhotoAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    return self;
}
-(void)addPhotoAction:(id)sender{
    [self.delegate parallaxViewAddPhotoButtonPressed];
}
