...
- (void) initialize
{
    ...
    [self initializePageControl]; //BY Saad
...
//    [_indicatorBackground addSubview:_countLabel];//By Saad to dont show the counter
...
//                }
            ...
            imageView.autoresizingMask=self.autoresizingMask;//By Saad
...
- (void) initializePageControl
{
    ...
    [_pageControl setHidden:YES];//By Saad
    //[self addSubview:_pageControl];
}
...
//    CGRect  visibleRect = CGRectMake(self.frame.size.width * nextPage, 0, self.frame.size.width, self.frame.size.width);
//    NSLog(@"image pager rect:%@",NSStringFromCGRect(visibleRect));
...
-(void)changeTheContentModeOfImagesInScrollViewWithOption:(UIViewContentMode)mode{
