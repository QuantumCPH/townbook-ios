...
-(id)init{
    self = [super initWithNibName:@"BarcodeScannerVC" bundle:nil];
    if (self) {
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    [self.allowedBarcodeTypes addObject:@"org.iso.DataMatrix"];//By Saad
    [self.allowedBarcodeTypes addObject:@"org.ansi.Interleaved2of5"];
...
//    [self showBarcodeAlert:barcode];BY Saad
    [self performSelectorOnMainThread:@selector(callTheDelegate) withObject:nil waitUntilDone:NO];
}
-(void)callTheDelegate{
    if (self.foundBarcodes.count) {//By Saad
        Barcode * barCodeSelected=[self.foundBarcodes lastObject];
        [self.delegate addBarcodeWithNumber:barCodeSelected];
