...
@property (nonatomic, strong) ActionBlock showPictureBlock;//By Saad

@property (nonatomic, strong) CAShapeLayer *bgLayer;
@property (nonatomic, strong) UIButton *disclosureButton;
@property (nonatomic, strong) UIButton *showPictureButton;//By Saad
...
- (void)setupSubtitleLabel {
    _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-32.0f, 36.0f, 176.0f, 24.0f)];
...
- (void)setupShowPictureButton {//By Saad
    BOOL iOS7 = [[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f;
    UIButtonType buttonType = iOS7 ? UIButtonTypeSystem : UIButtonTypeCustom;
    _showPictureButton = [UIButton buttonWithType:buttonType];
    _showPictureButton.tintColor = [UIColor clearColor];

    _showPictureButton.frame = CGRectMake(0,
                                          12.0f,
                                          _imageView.frame.size.width,
                                          _imageView.frame.size.height);

    [_showPictureButton addTarget:self action:@selector(didTapShowPictureButton) forControlEvents:UIControlEventTouchDown];
    [self addSubview:_showPictureButton];
}
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    self.showPictureBlock = thumbnail.showPictureBlock;
}
...
#pragma mark - ShowPicture Button

- (void)didTapShowPictureButton {//By Saad
    if (self.showPictureBlock) self.showPictureBlock();
}
@end
