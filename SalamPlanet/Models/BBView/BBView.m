...
-(void)fillBubbleViewWithButtons:(NSArray *)strings bgColor:(UIColor *)bgColor textColor:(UIColor *)textColor fontSize:(float)fsize {
    ...
    if (self.subviews.count == 0) {
        ...
        for (int xx = 0; xx < strings.count; xx++) {
            ...
            //Create Badge for each Button By Saad
            CustomBadge *crossBadge = [CustomBadge customBadgeWithString:@"x"
                                         ];
            crossBadge.frame=CGRectMake(buttonRect.origin.x+buttonRect.size.width, buttonRect.origin.y,15, 15);
            ...
            [self addSubview:crossBadge];//By Saad to show the cross image
        ...
        [self addBubbleButtonsWithInterval:0.0000001];
    }
